...
{% /group %}`,
  },
  {
    group: "Mechanism",
    selectWhen:
      "Use when the change carries an algorithm or non-obvious logic worth explaining. Explain the solution in several sentences, add a mermaid fence when a picture makes the logic clearer, then pin the code range that proves each critical claim as a collapsed block under that claim. Skip the group for a documentation, configuration, or mechanical change.",
...
Then state one critical claim per paragraph, and pin the code range that proves it under the claim with collapsed=true.
{% /section %}
{% /group %}`,
  },
  {
